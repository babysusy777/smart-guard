...
#include "fall_prototype_model.h"
...
static void calculate_signal_features( const float *signal, uint8_t n, float *features){
  ...
  if(n > 1) {
    features[5] =
      absolute_difference_sum / (float)(n - 1);
  } else {
    features[5] = 0.0f;
  }
...
static void extract_features( const patient_sample_t *window, uint8_t window_size, float *features){
  ...
  for(i = 0; i < window_size; i++) {
    adxl_magnitude[i] = square_magnitude(window[i].adxl_x, window[i].adxl_y, window[i].adxl_z);

    gyro_magnitude[i] = square_magnitude(window[i].itg_x, window[i].itg_y, window[i].itg_z);

    mma_magnitude[i] = square_magnitude(window[i].mma_x, window[i].mma_y, window[i].mma_z);
  }

  calculate_signal_features(adxl_magnitude, window_size, &features[0]);

  calculate_signal_features(gyro_magnitude, window_size, &features[FEATURES_PER_SENSOR]);

  calculate_signal_features(mma_magnitude, window_size, &features[2 * FEATURES_PER_SENSOR]);
}

static float calculate_fall_score(const float *features) {
  uint8_t prototype_id;
  uint8_t feature_id;

  float best_distance = FLT_MAX;

  for(prototype_id = 0;
      prototype_id < FALL_MODEL_NUM_PROTOTYPES;
      prototype_id++) {

    float distance = 0.0f;

    for(feature_id = 0;
        feature_id < FALL_MODEL_NUM_FEATURES;
        feature_id++) {

      float scale =
        FALL_PROTOTYPE_SCALE[prototype_id][feature_id];

      float normalized_difference;
      ...
      normalized_difference =
        (
          features[feature_id]
          - FALL_PROTOTYPE_MEAN[prototype_id][feature_id]
        ) / scale;
    ...
    distance /= (float)FALL_MODEL_NUM_FEATURES;
...
int tinyml_predict_window( const patient_sample_t *window, uint8_t window_size){
  ...
  last_score = calculate_fall_score(features);

  
  printf(
    "TinyML score=%ld threshold=%ld\n",
    (long)(last_score * 1000.0f),
    (long)(FALL_DISTANCE_THRESHOLD * 1000.0f)
  );

  if(last_score <= FALL_DISTANCE_THRESHOLD) {
